...
use Talav\CoreBundle\Interfaces\DumpableInterface;
use Talav\CoreBundle\Traits\PreventInitialTrait;
use stdClass;
use Traversable;
...
abstract class TypeCast
{
     ...
     * @since  __DEPLOY_VERSION__
     */
    public static function try(mixed $value, string $type, mixed $default = null, bool $strict = false): mixed
    {
        ...
        switch (strtolower($type)) {
            ...
            case 'float': // @todo: check
                ...
                return preg_match('~^-?\d+(\.\d+)?$~', $value) ? floatval($value) : $default;
